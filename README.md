...
- [ ] `POST /api/v1/logout` regenerates the token (invalidates the old one)
...
**Tasks**
- [ ] Migration: add `role` (enum, default `customer`) to `users`
- [ ] `require_admin!` before_action helper in `BaseController`
- [ ] Seed one admin user in `db/seeds.rb`
